(ns kopdarbackoffice.handlers.categories-one
  (:require [inertia.middleware :as inertia]
            [kopdarbackoffice.models.categories-one :as db]
  ...
  (fn [{:keys [params query-string uri] :as request}]
    (let [filters (select-keys params [:search :trashed])
          ...
          props {:categories {:data categories
                 ...
                 :filters filters}]
      (inertia/render "CategoriesOne/Index" props))))

(defn category-form [_]
  (inertia/render "CategoriesOne/Create"))
...
(defn edit-category!
  ...
  (fn [{:keys [path-params]}]
    (let [props {:category (db/get-category-by-id db (:category-id path-params))}]
      (inertia/render "CategoriesOne/Edit" props))))

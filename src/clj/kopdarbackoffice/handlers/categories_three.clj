(ns kopdarbackoffice.handlers.categories-three
  (:require [inertia.middleware :as inertia]
            [kopdarbackoffice.models.categories-three :as db]
  ...
  (fn [{:keys [params query-string uri] :as request}]
    (let [filters (select-keys params [:search :trashed])
          ...
          props {:categories {:data categories
                 ...
                 :filters filters}]
      (inertia/render "CategoriesThree/Index" props))))

(defn category-form [_]
  (inertia/render "CategoriesThree/Create"))

(defn store-category!
  [db]
  (fn [{:keys [body-params] :as req}]
    (if-let [errors (first (st/validate body-params category-schema))]
      (-> (rr/redirect "/categories-three/create")
...
(defn edit-category!
  ...
  (fn [{:keys [path-params]}]
    (let [props {:category (db/get-category-by-id db (:category-id path-params))}]
      (inertia/render "CategoriesThree/Edit" props))))

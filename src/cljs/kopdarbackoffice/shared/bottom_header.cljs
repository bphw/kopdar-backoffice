(ns kopdarbackoffice.shared.bottom-header
  ...
  (let [{:keys [^js auth]} (j/lookup (.-props (usePage)))
    ...
    (fn []
      [:div
       ...
       [:div {:class "mt-1 mr-4"} (.. user -company -nama)]
       [:div {:class "relative"}
        [:div
         ...
         [:div
          ...
          ;[:span (.-nama user)]
          [:span {:class "hidden ml-1 md:inline"} (.-nama user)]]
         [icon
          {:class

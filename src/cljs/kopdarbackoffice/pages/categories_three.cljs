(ns kopdarbackoffice.pages.categories-three
     ...
     [:div {:class "flex items-center justify-between mb-6"}
      ...
      [:> InertiaLink
       {:class "btn-indigo focus:outline-none",
        :href (js/route "categories-three.create")} [:span "Create "]
       ...
       [:tbody
        (for [etalase data
          ...
          [:tr {:class "hover:bg-gray-100 focus-within:bg-gray-100"
                :key tbid}
           [:td {:class "border-t"}
            [:> InertiaLink {:href (js/route "categories-three.edit" tbid)
...
(defn create-form [^js etalases]
  (let [{:keys [data setData errors post processing]}
        ...
        on-submit #(do (.preventDefault %)
                       (post (js/route "categories-three.store")))]
    [:div
...
(defn edit-form [^js cat]
  (let [{:keys [data setData errors put processing]}
        (j/lookup (useForm #js {
                                ...
                                :stde (or (.-stde cat) "")}))
        on-submit #(do (.preventDefault %)
                       (put (js/route "categories-three.update" (.-tbid cat))))
        destroy #(when (js/confirm "Are you sure you want to delete this etalase?")
                   (.delete Inertia (js/route "categories-three.destroy" (.-tbid cat))))
        restore #(when (js/confirm "Are you sure you want to restore this etalase?")
                   (.put Inertia (js/route "categories-three.restore" (.-tbid cat))))]
    [:div
     [:> Head {:title (str (j/get cat :kode) " " (j/get cat :nama))}]
     [:h1 {:class "mb-8 text-3xl font-bold"}
      [:> InertiaLink {:href (js/route "categories-three")

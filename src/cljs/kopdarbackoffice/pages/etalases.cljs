(ns kopdarbackoffice.pages.etalases
...
(defn index [{:keys [etalases]}]
  (let [{:keys [data links]} (j/lookup etalases)]
    [:div
     [:> Head {:title "Etalase"}]
     [:h1 {:class "mb-8 text-3xl font-bold"} "Etalase"]
     [:div {:class "flex items-center justify-between mb-6"}
      [:f> search-filter]
      [:> InertiaLink
       {:class "btn-indigo focus:outline-none",
        :href (js/route "etalases.create")} [:span "Create "]
       [:span {:class "hidden md:inline"} "Etalase"]]]
       ...
       [:tbody
        (for [etalase data
          ...
          [:tr {:class "hover:bg-gray-100 focus-within:bg-gray-100"
                :key tbid}
           [:td {:class "border-t"}
            [:> InertiaLink {:href (js/route "etalases.edit" tbid)
...
(defn create-form [^js etalase]
  (let [{:keys [data setData errors post processing]}
        ...
        on-submit #(do (.preventDefault %)
                       (post (js/route "etalases.store")))]
    [:div
     [:> Head {:title "Create Etalase"}]
...
(defn edit-form [^js etalase]
  (let [{:keys [data setData errors put processing]}
        (j/lookup (useForm #js {
                                :kode (or (.-kode etalase) "")
                                :nama (or (.-nama etalase) "")
                                :dsc1 (or (.-dsc1 etalase) "")
                                :dsc2 (or (.-dsc2 etalase) "")
                                :dsc3 (or (.-dsc3 etalase) "")
                                :stde (or (.-stde etalase) "")}))
        on-submit #(do (.preventDefault %)
                       (put (js/route "etalases.update" (.-tbid etalase))))
        destroy #(when (js/confirm "Are you sure you want to delete this etalase?")
                   (.delete Inertia (js/route "etalases.destroy" (.-tbid etalase))))
        restore #(when (js/confirm "Are you sure you want to restore this etalase?")
                   (.put Inertia (js/route "etalases.restore" (.-tbid etalase))))]
    [:div
     [:> Head {:title (str (j/get etalase :kode) " " (j/get etalase :nama))}]
     [:h1 {:class "mb-8 text-3xl font-bold"}
      [:> InertiaLink {:href (js/route "etalases")

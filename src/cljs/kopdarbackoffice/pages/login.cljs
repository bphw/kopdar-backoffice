(ns kopdarbackoffice.pages.login
  (:require ["@inertiajs/inertia-react" :refer [useForm Head]]
            [applied-science.js-interop :as j]
            [kopdarbackoffice.shared.buttons :refer [loading-button]]
            [kopdarbackoffice.shared.form-input :refer [text-input]]
            [kopdarbackoffice.shared.logo :refer [logo]]))

(defn login-form []
  (let [{:keys [data setData errors post processing]} (j/lookup
                                                       (useForm #js {:email "firice6969@eryod.com"
                                                                     :password "initial"
                                                                     :remember false}))
